				...
				{posts.map(post => (
					<li key={post.node.id} className="media">
						...
						<div className="media-body">
							...
							<small className="font-weight-lighter font-italic">{post.node.frontmatter.date}</small>
							{post.node.frontmatter.tags ? (
								<div className="btn-group btn-group-sm">
									{post.node.frontmatter.tags.map(tag => (
										<Link key={tag + `tag`} className="btn btn-info mr-1" to={`/tags/${kebabCase(tag)}/`}>{tag}</Link>
									))}
								</div>
							) : null}
						</div>
